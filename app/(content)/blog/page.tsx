...
export const metadata: Metadata = {
  title: 'Blog',
  description:
    'Some tech stuff and my life updates. I will post my writing here (mirroring too with my medium account).',
};
  ...
  return (
    <section className="md:mx-12">
      <div className="space-y-2 md:space-y-4">
        ...
        <p>
          I will post my writing here (mirroring too with my medium account).
          Stay tune, let me sleep first.
        </p>
      </div>
      {filteredBlogPosts.length ? (
        <div className="mt-4 grid grid-cols-1 gap-6 lg:grid-cols-2">
          {filteredBlogPosts.map(async (frontMatter, i) => {
            const base64 = frontMatter.image
              ? await getImage(frontMatter.image)
              : '';
            return (
              <Link
                key={i}
